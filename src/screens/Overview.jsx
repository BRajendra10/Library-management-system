...
import BookRequestCard from "../components/RequestCard";
...
function Overview() {
  ...
  const requestbooks = useSelector((state) => state.requestbooks.requestbooks);
      ...
      <div className="col-span-12 h-full grid grid-cols-12 gap-5 px-5">
        <section className="col-span-6 h-[35rem] flex flex-col border border-blue-200 rounded-lg">
          ...
          <div className="w-full h-full overflow-scroll">
            {overDueBooks.map((el, inedx) => (
              <OverdueCard id={inedx} data={el} />
            ))}
          </div>
        </section>
        ...
        <section className="col-span-6 h-[35rem] flex flex-col border border-blue-200 rounded-lg">
          ...
          {/* details */}
          <div className="w-full h-full overflow-scroll">
          {requestbooks.map((el) => <BookRequestCard data={el} />)}
          </div>
        </section>
      </div>
    </div>
